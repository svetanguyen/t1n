...
const Contact = () => {
  ...
  return (
    <div className="min-h-screen px-4 lg:px-11 bg-gradient-top-50 pt-[116px] lg:py-[180px] lg:flex items-center">
        <form className="bg-gray-100 shadow-1 mb-16 lg:mb-0 p-4 justify-between flex flex-wrap lg:p-12 shadow-[0px_20px_40px_rgba(0, 0, 0, 0.1)]" onSubmit={sendEmail}>
            <input onChange={(e) => setName(e.currentTarget.value)} value={name} placeholder="name" className="font-lightexp focus-visible:outline-0 focus:border-b placeholder:lg:text-4xl placeholder:font-lightexp placeholder:text-dark-200 placeholder:opacity-40 text-sm lg:text-4xl lg:h-14 border-b w-full lg:w-[49%] block border-black-opacity bg-transparent" type="text" name="user_name" />
            <input onChange={(e) => setMail(e.currentTarget.value)} value={mail} placeholder="e-mail" className="font-lightexp focus-visible:outline-0 focus:border-b placeholder:lg:text-4xl placeholder:font-lightexp placeholder:text-dark-200 placeholder:opacity-40 text-sm lg:text-4xl lg:h-14 border-b w-full lg:w-[49%] mt-4 lg:mt-0 block border-black-opacity bg-transparent" type="email" name="user_email" />
            <textarea onChange={(e) => setMessage(e.currentTarget.value)} value={message} placeholder="message" className="font-lightexp focus-visible:outline-0 lg:p-4 placeholder:lg:text-4xl lg:text-4xl placeholder:font-lightexp placeholder:text-dark-200 placeholder:opacity-40 text-sm w-full p-1 mb-10 lg:mb-14 mt-5 lg:mt-8 h-96 bg-transparent border border-black-opacity" name="message" />
